...
const getUsuarios = async (req, res, next) => {
  try {
    ...
    const result = await pool.query("SELECT *FROM usuario WHERE idusuario = $1", [
      idusuario,
    ]);
...
const crearUsuarios = async (req, res, next) => {
  try {
    ...
    const { nombre_u, apellido_u, edad, telefono, email, correo, direccion, password, rol_idrol} = req.body;
    const result = await pool.query(
      ...
      [ nombre_u, apellido_u, edad, telefono, email, correo, direccion, password, rol_idrol]
    );
...
const actualizarUsuarios = async (req, res, next) => {
 ...
 try{
    ...
    const result = await pool.query(
      ...
      [nombre_u, apellido_u, edad, telefono, email, correo, direccion, password, rol_idrol, idusuario]
    );
    if (result.rows.length === 0)
      return res.status(404).json({
        message: "Usuario no encontrado",
...
const eliminarUsuarios = async (req, res) => {
  const { idusuario } = req.params;
  try{
    const result = await pool.query("DELETE FROM usuario WHERE idusuario = $1", [
        idusuario,
      ]);
