...
const getproducto = async (req, res, next) => {
  try {
    ...
    const result = await pool.query("SELECT *FROM producto WHERE idu_producto = $1", [
      id_producto,
    ]);
...
const crearproducto = async (req, res, next) => {
  try {
    ...
    const { descripcion_prod, peso_prod, fechavencimiento, nombre_prod} = req.body;
...
const eliminarproducto = async (req, res) => {
  const { id_producto } = req.params;
  try{
    const result = await pool.query("DELETE FROM producto WHERE id_producto = $1", [
        id_producto,
      ]);
